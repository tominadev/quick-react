...
const applyMigrations = async (database) => {
	database.exec('CREATE TABLE IF NOT EXISTS global_schema_migrations (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, deleted_at INTEGER NOT NULL DEFAULT 0, created_duid INTEGER, updated_duid INTEGER, owner_tid INTEGER NOT NULL DEFAULT 1, owner_uid INTEGER, migration_key TEXT NOT NULL UNIQUE, applied_at INTEGER NOT NULL)');
