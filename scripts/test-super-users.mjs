...
try {
	...
	assert.equal((await app.request('http://localhost/api/panel/admin/base/audit/records.php?action=approve', { method: 'POST', headers: superHeaders, body: JSON.stringify(queued.table.dataSource.map((row) => String(row.id))) })).status, 200);
	// 重名的建号要在**记录之前**挡掉：审批是先记录后应用，等 INSERT 撞索引才失败的话，
	// 队列里会留下一条指向从未写成的行的申请，批也批不动。
	const queuedNow = async () => (await (await app.request('http://localhost/api/panel/admin/base/audit/records.php?include=data&review_status=pending', { headers: superHeaders })).json()).table.dataSource.length;
	...
	const approve = async (headers) => {
		const pending = await (await app.request('http://localhost/api/panel/admin/base/audit/records.php?include=data&review_status=pending', { headers })).json();
		const ids = pending.table.dataSource.map((row) => String(row.id));
		const response = await app.request('http://localhost/api/panel/admin/base/audit/records.php?action=approve', { method: 'POST', headers, body: JSON.stringify(ids) });
	...
	const rejectedIds = await (async () => {
		const list = await (await app.request('http://localhost/api/panel/admin/base/audit/records.php?include=data&review_status=pending', { headers: superHeaders })).json();
		const ids = list.table.dataSource.map((row) => String(row.id));
		assert.equal((await app.request('http://localhost/api/panel/admin/base/audit/records.php?action=reject', { method: 'POST', headers: superHeaders, body: JSON.stringify(ids) })).status, 200);
		return ids;
	})();
	const requeued = await app.request('http://localhost/api/panel/admin/base/audit/records.php?action=requeue', { method: 'POST', headers: superHeaders, body: JSON.stringify(rejectedIds) });
