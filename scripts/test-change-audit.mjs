...
try {
	...
	assert.equal((await transitionAuditEntries(acting, [(await latestEntry()).id], 'revert'))[0].ok, true, '写入字符串的那条也要能回滚');
	...
	assert.equal((await transitionAuditEntries(acting, [arrayEntry.id], 'revert'))[0].ok, true, '数组列必须能回滚');
