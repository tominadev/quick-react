...
const fingerprintData = JSON.stringify({ canvas_cyrb53: '4b5a6c7d8e9f', audio_cyrb53: '1a2b3c4d5e6f' });
	...
	// 电脑打开二维码页。
	const qr = await (await app.request('https://accounts.test/api/accounts/external/wechat?format=json', { headers: { cookie: 'passport_session=desktop-session', 'x-device-key': deviceKey, 'x-device-fingerprint': fingerprintData } })).json();
	...
	assert.match(document, /<title>示例账户中心<\/title>/);
	assert.match(document, /账户中心/);
	...
	const standalone = await (await app.request(`https://accounts.test${secondQr.pollUrl}`, { headers: { 'x-device-key': deviceKey, 'x-device-fingerprint': fingerprintData } })).json();
	...
	assert.deepEqual(await (await app.request(`https://accounts.test${thirdQr.pollUrl}`, { headers: { 'x-device-key': deviceKey, 'x-device-fingerprint': fingerprintData } })).json(), { status: 'authenticated', redirectTo: '/panel/accounts.html' });
	...
	const continued = await app.request(`https://accounts.test${fourthQr.pollUrl}`, { headers: { cookie: 'accounts_oidc_request=qr-request', 'x-device-key': deviceKey, 'x-device-fingerprint': fingerprintData } });
	...
	const expiredPoll = await app.request(`https://accounts.test${fourthQr.pollUrl}`, { headers: { 'x-device-key': deviceKey, 'x-device-fingerprint': fingerprintData } });
