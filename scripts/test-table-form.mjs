...
try {
	...
	// 两条规则各测一次：
	// 1) 系统字段（id、created_at 等）由 resolveTableFormColumns 统一剔除，新增和编辑都不出现；
	// 2) 非系统字段用 form.create / form.edit 做场景覆盖，false 表示该场景不显示。
	const columns = [
		{ dataIndex: 'id', title: 'ID', component: 'textbox' },
		{ dataIndex: 'code', title: '编号', component: 'textbox', form: { edit: false } },
	...
	assert.deepEqual(resolveTableFormColumns(columns, 'create'), [
		{ dataIndex: 'code', title: '编号', component: 'textbox' },
