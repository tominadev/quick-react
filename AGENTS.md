...
- 数据表统一使用 `id` 作为本表主键，并强制包含以下五个 `NOT NULL` 字段：`id BigInt @id @default(autoincrement())`、`created_at BigInt`、`updated_at BigInt`、`created_duid BigInt`、`updated_duid BigInt`。其中 `duid` 明确定义为 `device_user_id`；新增时五个字段均必须有值，更新时只自动更新 `updated_at`、`updated_duid`，不得修改创建字段。无设备或系统操作使用固定 `duid=0`。跨表引用使用被引用实体的语义名称加 `_id`（例如 `passport_user_emails.id` 在其他表中使用 `user_email_id`），原业务唯一字段保留为唯一约束而非主键；安全令牌和外部标识仍保留字符串类型。
