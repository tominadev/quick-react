...
- **平台的角色是「可信公钥登记处」，不是密钥保险箱。** 平台不铸造、不下发、不保管任何用来证明「这次调用是对方发起的」的秘密；它维护的是一份**谁对应哪把公钥**的名单，并为这份名单背书。对方在自己的机器上生成 Ed25519 密钥对，把公钥主动填到平台；平台据公钥反查身份（公钥全局唯一），验签过了才算数。样板是 `sms_integration_client_keys.public_key`：接入方签绑定票据，平台只存公钥。
  ...
  **名单里只发公钥，不发密钥标识。** 判定来源的从头到尾是公钥本身；多给一个由公钥算出来的标签（`kid`），只会诱使对方拿它当缓存的键，而那正是「轮换之后突然全验不过」的来源。标签留在数据库和管理页面上供人称呼，不进协议。

  **过期时间挂在名单上，不挂在单把密钥上。** 对方需要知道的是「我这份缓存还能信多久」，那是一个缓存时长（`max_age_seconds`，同时写进 `Cache-Control`）。给每把公钥标一个到期时刻是一句守不住的承诺：退役是人点出来的，不是到点自动发生的，而密钥一旦泄露必须能立刻换掉——事先公布的时刻在那一刻就作废了。对方信了它，早退役会把真请求当成伪造丢掉，晚退役会继续信一把已经不该信的公钥。**这个缓存时长同时就是轮换的节拍**：先公布、等满一个时长、再启用签名，界面上那句「等 N 分钟」与端点公布的数必须出自同一个常量。
