...
- 数据表字段顺序固定：前六个字段必须依次为 `id BigInt @id @default(autoincrement())`、`created_at BigInt`、`updated_at BigInt`、`deleted_at BigInt @default(0)`、`created_duid BigInt?`、`updated_duid BigInt?`，随后紧接两个可空业务归属字段 `owner_bid BigInt?`、`owner_uid BigInt?`（归属由粗到细，先分站后账号），再放其他业务字段。所有表（包括 `base_devices`、`base_device_users`、`base_device_snapshots` 等设备来源表）都必须保留软删除字段和两个可空审计字段。其中 `duid` 明确定义为 `device_user_id`，`bid` 明确定义为 `branch_id`（分站，指向 `base_branches.id`；分站不是 `global_sites` 的代码站点，命名一律不使用 `site`）；前六个字段是公共层管理的系统字段，后台和用户表单只能展示，禁止新增或编辑写入。`owner_bid`、`owner_uid` 不属于审计系统字段：新增时公共层按当前作用账号及其分站自动填充，系统或无法获取时为 `NULL`，后续允许通过业务过户操作修改；业务层按约定不写这两个字段——公共层填充的已经是正确值。`owner_bid` 创建时定死，账号之后调去别的分站不改变旧数据归属。新增时由公共层写入时间字段，更新时只自动更新 `updated_at`、`updated_duid`，不得修改创建字段；软删除和恢复只能调用公共层专用操作。普通设备操作写入真实 `device_user_id`，系统、机器人或无设备操作写入 `NULL`。跨表引用使用被引用实体的语义名称加 `_id`（例如 `passport_user_emails.id` 在其他表中使用 `user_email_id`），原业务唯一字段保留为唯一约束而非主键；安全令牌和外部标识仍保留字符串类型。
