...
- **重命名字段和修改字段类型会丢数据**，这两种是 `migrate diff` 的固有限制：它只比对两份 schema 的最终形态，看到的是「旧列没了、多了个新列」，无从知道这是同一列，因此生成的是 `DROP COLUMN` + `ADD COLUMN`（SQLite 则是拷贝时把那一列从 `SELECT` 里漏掉）。`prisma migrate dev` 会交互式追问是不是重命名，但那条路这个项目用不了：多 schema、多方言、目标库要到运行时才从 `global_sites.dsn` 取。数据废弃就直接用生成的迁移；要保留则改成两步——先加新列生成一个迁移，再单独写一个回填迁移，最后删旧列——或者在生成的迁移里人工补拷贝语句并写清原因。
