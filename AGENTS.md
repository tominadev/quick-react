...
- 加字段、改字段、删字段都由 Prisma 自动生成，不要自己写 SQL。SQLite 不支持的原地修改，Prisma 会自动改用「建新表 → 拷数据 → 换名 → 重建索引」，数据不丢。只有 Prisma 表达不了的场景（例如新增 `NOT NULL` 且无默认值的列需要回填历史行）才需要人工补一个迁移文件，并在文件里写清为什么。删字段时数据一并丢弃是预期行为，需要保留就先加新列迁移数据再删。
