...
2. 在那一行点**公钥**，登记一把 Ed25519 公钥。`名称` 随便起（如「生产服务器」），只是给人看的标签。
   ...
   **这把公钥就是你的身份**，和 GitHub 放 SSH 公钥是一回事：票据里带上它，SMS 据此反查出是哪个接入方、手机该登记到谁名下——你不用再填接入方标识、账号 id 之类的东西。`名称` 只是给人看的标签，不进协议。

   一把公钥全平台只能登记一次。换钥匙时登记新的一把，两把并存一段时间，再把旧的改成「已退役」。
...
> **为什么填公钥是安全的？** 公钥本来就不是秘密。填成别人的公钥，就得拿**别人的私钥**来签才验得过——而私钥从不出签发方的门。所以你只可能以「你自己」的身份绑定，越不了权，也不存在填错成别人的可能。
