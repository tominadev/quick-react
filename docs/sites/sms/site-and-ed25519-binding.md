...
因此轮换期间**两把并存**，在管理后台的「推送密钥」页操作：

1. 点「生成新密钥」——新的一把成为 `active` 并立即用于签名，当前那把自动转 `retiring`；
2. 两把公钥同时出现在 `/api/push-key`，接收方按 `kid` 挑；
3. 等过一个完整的投递重试窗口——**观察时间必须盖过重试窗口**，否则最后几次重试仍会验不过；
4. 在 `retiring` 那一行上点「退役」，它不再公布。

**先插新的、再把旧的转 `retiring`**，两条语句一次提交。反过来的话中间会有一瞬间一把 `active` 都没有，那时候来的短信签不了名、投递直接失败；而这个顺序的中间态是两把 `active`，签名取最新那把，不影响任何一次投递。
