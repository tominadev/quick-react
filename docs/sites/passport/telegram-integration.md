...
> 2026-08-28 修订：身份源**自己提供已验证邮箱**（目前只有 Google 的 `email_verified`）且该邮箱已经是某个启用中账号的已验证邮箱时，改为直接把该外部身份绑定到那个账号并登录，不再要求用户先用原方式登录。理由见 [account-center](account-center.md#按已验证邮箱自动关联身份2026-08-28-决策)。其它情况（微信等不提供邮箱的身份源走我方验证码、外部身份已绑定到另一个账号）仍然保持不自动合并。邮箱、Telegram 和其他 Provider 的数量上限通过 Passport 设置项控制，默认不限制。
...
```text
/opt/example/php-telegram-iam/couchdb-backup-iam-20260826-005734.json
```
...
旧项目头像暂不迁移。后续迁移时，头像源目录为 `/opt/example/php-telegram-iam/wwwroot/assets/avatars`，旧文件名按 Telegram 用户 ID 保存，需根据旧 `tgfromid` 与 `userid` 关系转换为 `avatars/<user_id>.<ext>`。旧项目配置中的机器人 Token 不属于 CouchDB 备份，机器人配置应通过 global 机器人管理功能单独迁移或重新录入。
