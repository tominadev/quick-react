...
复盘记的是某一天发生过的事，写完基本不再修订。**日期写在文内的「记录日期」和下面这张表里，
不进文件名**——全仓一个规矩：文件名说主题，日期放文档里。

| 文档 | 主题 | 记录 |
| --- | --- | --- |
| [table-switch-stale-ui](postmortems/table-switch-stale-ui.md) | 数据管理切换表格时 UI 混合问题 | 2026-08-29 |
...
- **每篇需求文档的标题下面紧跟一个头部块**，顺序固定，`上游需求` 没有就不写：
  ...
  **`状态` 要能被推翻。** 写「已实施」就要能指出代码在哪；部分实施就写清哪几节没做；说不准的写
  ...
  日期一律绝对日期。**文件名里不放日期**，全仓都不放：文件名说主题，日期放文档头部和索引表里。
