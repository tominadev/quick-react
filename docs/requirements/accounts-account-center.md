...
- 退出登录由当前站点后端撤销 Accounts 总会话并触发 OIDC Back-Channel Logout；成功响应通过通用 `next: { action: 'navigate', path, refreshAuth: true }` 明确要求重新读取认证与导航状态，再在原路径软切换。受保护路径会由后端返回 401 状态页，让用户明确知道已经退出；前端只执行后端指令，不自行推断跳转目标或登录状态。
