...
## `queued_at`：为什么不叫 `pended_at`

`_at` 这套命名的模式是**过去分词 + `_at` = 这件事发生的时刻**：`created_at`、`updated_at`、
`deleted_at`、`settled_at`。

`pend` 在现代英语里几乎只活在 `pending` 里，及物用法基本不用，`pended` 是生造出来的过去
分词。改成 `pending_at` 更不成句——`pending` 是形容词，读作「在等待中的时刻」，与其余四个
「被怎样了的时刻」对不上，只是把一个别扭换成另一个别扭。

`queue` 是标准及物动词，`queued` 套得进那个模式，语义也更准：这一行**进了队列**等审批。
它还与审批记录上的 `settled_at` 天然配对——进队列 / 出队列。写那两列的注释时反复用的就是
「还在队列里」「进队列」「离开队列」，`queue` 本来就是这件事的自然词汇，只是列名一直没跟上。

**顺带把两个 pend 分开了。** 队列是**行**的状态（`queued_at`：这一行看不看得见），
`pending` 是**申请**的状态（`review_status`：这条申请批没批）。它们相关——行在队列里等价于
有一条 pending 的新建申请——但共用一个词根时，读代码得靠上下文分辨这个 "pend" 说的是哪一头。
