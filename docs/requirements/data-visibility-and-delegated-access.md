...
前置需求：[数据行归属](row-level-data-ownership.md)。该需求已于 2026-09-01 实施，为每张表提供了 `owner_uid` 归属字段和公共层的归属填充逻辑。本需求在其之上增加**分站维度、可见性判定与代用户操作**。
...
同时补入一个此前遗漏的维度：业务站点内部存在**分站**，每个分站有自己的客服，分站之间数据隔离。分站不是 `global_sites` 的代码站点，两者不可混淆。

## 2. 目标

- 每张表增加 `owner_bid`，标识该行归属的分站。
- 账号只能看到和改动归属于自己的数据。
- 授权角色可以显式指定一个目标账号，代其查看和操作，**限本分站内**。
...
- 分站实体表为 `base_branches`，归属字段为 `owner_bid`，普通跨表引用为 `branch_id`。这与既有的 `owner_uid`（归属）/ `base_user_id`（跨表引用）是同一个模式。
- **不使用 `site` 一词**：`global_sites` 已经表示代码站点，再用 `site_id` 表示分站会造成歧义。
...
- `bid` 需要在 AGENTS.md 中显式定义为 `branch_id`，与既有的"`duid` 明确定义为 `device_user_id`"同例。
- 复数拼写为 `branches`，不是 `branchs`。

## 4. 数据结构变更

### 4.1 `owner_bid`

位置固定为第 7 位，**排在 `owner_uid` 之前**：归属由粗到细，先分站后账号。

| 字段 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `owner_bid` | `BigInt?` | 由公共层在 INSERT 时写入 | 归属分站，指向当前数据库的 `base_branches.id`；`NULL` 表示不属于任何分站 |
...
**写入语义：创建时定死。** 数据属于创建它时所在的分站；账号之后调去别的分站，不改变旧数据的归属。因此不需要任何回写——若改为"跟随属主当前分站"，账号换分站就要修改它在全部 58 张表中的所有行，而 D1 没有事务，中途失败会留下部分表归属陈旧的状态，把一个组织问题换成了正确性问题。

### 4.2 `base_branches`

| 字段 | 说明 |
| --- | --- |
| `key` | 分站标识，`(key, deleted_at)` 唯一 |
| `name` | 显示名称 |
| `status` | `enabled` / `disabled`；停用后其账号不能登录，数据保留 |

分站记录本身属于平台，`owner_uid` 与 `owner_bid` 均为 `NULL`，因此按 §5.2 对普通账号不可见，由 `super` 在后台管理。

### 4.3 `base_branch_hosts`
...
| `branch_id` | 指向 `base_branches.id` |
| `status` | `enabled` / `disabled` |

### 4.4 主机名的两阶段解析

边界规则一句话：**global 只认站点，站点库只认分站，谁都不越界。**

| 问题 | 谁回答 | 存哪 |
| --- | --- | --- |
| 这个请求用哪套代码、哪个库 | 控制面 | `global_site_hosts` |
| 这个请求属于哪个分站 | 站点自己 | `base_branch_hosts` |

同一个主机名会在两张表里出现，但回答的是两个不同的问题，不是同一事实抄两遍。**`global_site_hosts` 不增加任何分站字段。**
...
2. **站点库**：用同一个主机名查 `base_branch_hosts` 得到 `branch_id`。数据库此时已经选定，所以这一步不参与站点解析，不违反"主机名解析必须先于开库"这条约束。

#### 未匹配到分站时的处理

控制面有两条兜底路径——通配符与 `defaultSiteKey`——意味着**未登记的主机名照样能到达站点**。任何人把域名 CNAME 过来，或访问 `anything.sms.example.com`，都会进入站点而查不到分站。若一律按"无分站"放行，就能在上面注册出 `owner_bid` 为 `NULL` 的账号。因此按控制面的命中方式区分：

| 控制面命中方式 | 站点库无分站匹配时 |
| --- | --- |
| `exactHosts` 精确行 | 允许，视为平台自有入口（主域名），`owner_bid` 为 `NULL` |
| 通配符或 `defaultSiteKey` 兜底 | **拒绝，返回 404** |

精确行是运维显式登记的，意图明确；通配符与默认兜底只是"顺便接住"，不应被当作合法的平台入口。这样无分站账号只可能产生在管理员明确开放的主域名上。

#### 写入顺序

跨库写入没有事务，顺序是强制的：

- **新增绑定：先站点库，后控制面。** 中断只留下"分站域名已登记但路由不通"，无害可重试。反序会留下"域名能路由过来但站点库不认识它"，访客落入未绑定状态。
...
| --- | --- | --- |
| 站点初始化 | 一行通配符 `*.<站点主域> → <站点>` | — |
| 新增分站 | **不动**，通配符已覆盖 | `base_branches` + `base_branch_hosts` 默认子域行 |
| 绑定自定义域名 | 精确行 | `base_branch_hosts` 一行 |

新增分站时自动写入一行 `<分站 key>.<站点主域>` 的默认子域，使解析只有**一套规则**（查表），不存在"查不到再按子域标签推断"的第二套规则。运维只有绑定自定义域名时才需要两边都写，而那是解析顺序决定的物理必需。

#### 缓存

项目原则是"hosts 整表加载到内存，普通请求不查询数据库"。`base_branch_hosts` 同样需要快照，**按数据库分别缓存**，TTL 与 `server/modules/base/site-router.mts:100` 一致（30 秒），并提供显式 `refresh()`。绑定或解除后必须主动刷新，否则运维会以为操作没生效。

这是选择 `base_branch_hosts` 而非在控制面加分站字段所付出的主要代价：多一层按库的缓存。换来的是控制面不需要知道分站的存在。

#### 孤儿行

`base_branch_hosts` 中的主机名若在控制面既无精确行、也不被通配符覆盖，该记录永远不会被命中，是条死数据。管理页绑定自定义域名时必须**先校验控制面可达**，不可达则提示，不得静默写入。

### 4.5 账号的分站归属

**一个账号只属于一个分站，一对一。** 不新增关联表，也不在 `base_users` 上新增 `branch_id`——`base_users.owner_bid` 本身就是该账号所属的分站。

该值在会话解析时顺带取出：`server/modules/base/auth/index.mts:115` 已经 join 了 `base_users` 并取 `u.id`、`u.name`、`u.roles`，把 `u.owner_bid` 一起 select 出来即可，不增加查询。

**本需求不新增任何账号字段。** 等级模型放弃后，`perm_level` 不再需要。

账号所属分站在注册时由 §4.4 的第二阶段解析确定：从当前主机名查到 `branch_id`，写入新账号的 `owner_bid`。管理员在后台建账号时则取管理员自己的分站。
...
- 默认等于登录账号。
- 具备代查角色的账号可以显式指定为**本分站内的**另一个账号，见 §6。

```ts
export type SqlSubject = {
  actingUid: bigint | null;   // 作用账号，默认等于登录账号
  actingBid: bigint | null;   // 作用账号所属分站
  superAdmin: boolean;        // roles 含 super：平台管理员，跨分站，不受判定约束
  branchAdmin: boolean;       // roles 含 admin：分站管理员，本分站全部
};
```
...
| 2 | 角色含 `admin`，且 `owner_bid` = 自己的分站 | 允许 | 允许 |
...
角色是集中常量，定义在 `shared/types/role.mts`，不进数据库。本需求新增 `super` 与 `support`，并把 `admin` 收窄为分站管理员：

| 角色 | 可分配 | 可见范围 | 谁能授予 |
| --- | --- | --- | --- |
| `super` | 是 | 全部，跨分站；管理 `base_branches` 与全部 `global_*` | `super` |
| `admin` | 是 | **本分站全部** | `super` |
| `support` | 是 | 自己的数据，加上代查本分站指定账号 | `super`、`admin` |
| `public` / `user` / `accounts` | 否（隐式） | `user` 只看自己的数据 | 运行时隐式授予 |

角色名不使用 `root`：Unix 里 root 是账号（uid 0）而非角色，用作角色名会误导。

`admin` 与 `support` 的区别是**广度与精度**：前者一次看到本分站全部数据，那是管理职责；后者一次只能指定一个账号且留审计记录，因此看不到全分站的横截面。

界面上不会出现光秃秃的角色键：`roleLabel`（`shared/types/role.mts:24-27`）按"中文名(英文键)"渲染，显示为「平台管理员(super)」「分站管理员(admin)」，范围一眼可见；`description` 中同样写明"仅限本分站"。

#### `admin` 语义变窄带来的两处强制改动

`admin` 从"平台管理员"变成"分站管理员"，是一次**静默的权限收窄**——既有代码不会报错，但边界移动了。以下两处必须一并处理，否则会留下真实缺陷：

**一、既有 `roles: ['admin']` 门要逐个改判。** 平台级入口改为 `roles: ['super']`，分站级入口保留 `admin` 或写成 `['super', 'admin']`。至少涉及 `server/routes/base/navigation.mts:17,25`、`server/routes/global/navigation.mts:17`。判断标准：控制面（`/panel/admin/global/*`）、分站管理页、数据库与站点管理归 `super`；本分站的账号与业务管理归 `admin`。

**二、救援入口必须改为授予 `super`。** `server/modules/base/maintenance/actions.mts:43` 目前恢复 `base_users.id = 1` 时写入 `admin`。该账号建立时没有会话，`owner_bid` 为 `NULL`，而收窄后 `admin` 的谓词是 `owner_bid = ?`——`owner_bid = NULL` 求值为 unknown，一行都查不到，救出来的会是一个什么都看不见的管理员。`super` 不追加任何谓词，救援才成立。这与维护文档中既有的 `rescue_superadmin` 救援上下文语义一致，见 [维护工具箱](maintenance-toolbox.md)。同理 `server/routes/base/api/sign.mts:63` 创建的初始管理员也应为 `super`。
...
**四种情况都是单个索引等值**，没有 `OR`、没有子查询、没有 JOIN。`admin` 走 `(owner_bid, deleted_at)` 索引，其余走 `(owner_uid, deleted_at)`。

对普通账号与 `support` 而言 `owner_bid` 不进谓词：作用账号的数据天然就在自己的分站里，追加分站条件是冗余的。分站条件只在 `admin` 这一支出现。
...
| 每张受管业务表 | `(owner_bid, deleted_at)` | 分站级聚合视图 |
...
新增记录不做行级判定，准入由现有角色机制控制。公共层自动写入 `owner_uid` = 作用账号、`owner_bid` = 作用账号所属分站。

## 6. 代用户操作

### 6.1 授权

由**角色**判定，不由等级判定。具备 `support` 角色的账号可以指定目标账号。`admin` 已能看到本分站全部数据，不需要代查。
...
- `owner_uid`、`owner_bid` 记**目标账号**及其分站——数据是目标账号的。
...
要指定目标账号就得先找到它，而 `base_users` 本身受行级判定管辖。因此账号查找必须是**显式授权的独立接口**，以系统上下文读取，按请求者的 `owner_bid` 限定范围，只返回选择目标所需的最小字段（id、用户名、状态）。它不依赖行级判定放行。
...
客服需要"看本分站全部数据"时，热路径谓词（`owner_uid = 作用账号`）做不到——它一次只对应一个账号。此类视图必须是**独立的授权接口**，以系统上下文查询并显式追加 `owner_bid = <请求者分站>`，自带角色校验。`admin` 不需要它——本分站全部数据已由 §5.3 的谓词覆盖。

不得为了聚合而放宽 §5.2 的规则。

## 8. 实施步骤

1. 新建 `base_branches` 与 `base_branch_hosts`，四个 Prisma schema 全部 58 个模型在 `owner_uid` 之前插入 `owner_bid`，补四个迁移组 × 四种方言的迁移文件。
2. 实现 §4.4 的第二阶段解析与按库缓存，含未匹配时按控制面命中方式区分放行或 404。
3. `server/modules/base/auth/index.mts:115` 的会话查询增加 `u.owner_bid`。
...
10. 分站管理页面（`/panel/admin/base/branches`，`super` 限定），含自定义域名绑定与控制面可达性校验。
...
- 全新初始化的数据库中，每张表都存在 `owner_bid` 列且位于 `owner_uid` 之前，`base_branches` 存在。
- 账号 A 新增记录后，`owner_uid` 为 A、`owner_bid` 为 A 所属分站；A 能查到、能改。
...
- 具备代查角色的客服指定同分站账号 A 后，能查到并能改动 A 的数据。
- 该客服指定**其他分站**账号时被拒绝。
...
- 账号查找接口只返回请求者本分站的账号。
- 非 `super` 账号读不到 `base_branches`。
- 在已绑定分站的主机名上注册的账号，`owner_bid` 等于该分站。
- 通配符命中但站点库无分站匹配的主机名返回 404，无法注册。
- 控制面精确登记的平台主域名可正常访问，其上注册的账号 `owner_bid` 为 `NULL`。
- 绑定自定义域名后刷新缓存，新域名立即可用；解除绑定后立即失效。
- 绑定一个控制面不可达的域名时被拒绝，站点库不产生记录。
- `admin` 能查到并改动本分站任意账号的数据，查询其他分站的数据返回 0 行。
- `admin` 进不了 `/panel/admin/global/*` 与分站管理页。
...
- **分站是一层，没有层级**：分站之间平级，不支持"总部看所有分站"。总部场景目前只能用 `admin`。
- **`owner_bid` 创建时定死**：账号调去别的分站后，其旧数据仍归属原分站。这是有意的，但需要在分站合并或调整时由运维显式迁移。
- **谓词只作用于主表**：`select` 的关联表可见性由业务查询自身保证。
- **未绑定主体即放行**：绕过机制的代价，靠"绑定点唯一"和冒烟用例约束，不靠类型系统保证。

## 12. 待定事项

- **`sign.mts:63` 的初始管理员**。它在数据库刚建好、还没有任何分站时创建，`owner_bid` 必然为 `NULL`。按 §5.2.1 它应当授予 `super`，因此 `NULL` 分站不影响其可见范围。但若将来允许非首次的自助注册走同一路径，需要按 §4.4 补上主机名解析。
