...
- 保存响应带着刷新过的身份（`user`）和认证上下文（`context.auth`）：改完用户名或昵称，页面上半截和右上角的昵称都立刻跟着变，不必再问一次接口。路由通过 `c.set('refreshAuthContext', true)` 显式声明要这份上下文；客户端在响应没有 `next` 时只应用其中的 `auth` 一项，不碰导航树和页面状态——这类响应并不携带它们，整份套上去会把它们清空。
