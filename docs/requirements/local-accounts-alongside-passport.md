...
**首次登录不同步密码。** 凭证 claim 是密码哈希，不能存进 `base_oidc_login_requests.claims`——那份 claims 最终会写进 `base_oidc_users.profile`，而那是「数据管理」里可见的普通列。下次 Accounts 登录会补上同步。

### 6.12 页面协议新增 `FormPageSection`
...
- `base_oidc_login_requests` 新增 `status`（`pending` / `choosing`）、`subject` 与 `claims`：待决期间暂存已验证的身份，落定时不再向 Accounts 二次换取。
