...
**一部手机、一份 Shortcut、一条短信只进来一次，再由服务端分发给各个项目。**

曾经考虑过另一种做法：把接入方放进 `sms_phones` 的唯一索引，让同一个号在每个项目下各绑一次，每个项目各自领一份 Shortcut。那样项目之间倒是完全隔离了，代价却是**同一部手机要装好几个快捷指令**，同一条短信被转发好几次，手机端跑好几次网络请求——为了服务端少一张关系表，把成本推给了用户的手机。

而那张关系表根本不需要：推送地址上同时有 `integration_client_id` 与 `phone_id`，用户为某个项目配一条限定这部手机的推送地址，**这个动作本身就是授权**。
