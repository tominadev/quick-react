...
| `number` | 规范化后的 E.164 手机号，`(owner_tid, owner_uid, integration_client_id, number, deleted_at)` 唯一。票据中的协议字段名为 `phone` |
| `title` | 用户可修改的设备名称 |
| `status` | `enabled` / `disabled` / `revoked` |
| `bound_at`、`revoked_at` | 绑定与撤销时间，后者可为空 |
| `integration_client_id` | 属于哪个接入方项目，`0` 表示不属于任何项目（用户在控制台自己绑的那一部）。用 `0` 而不是 `NULL`：它要进唯一索引，而 SQL 里 `NULL != NULL` |
...
**解绑不是删除。** `revoked` 的行 `deleted_at` 仍为 0，照样占着唯一索引，因此「重新绑定」是**复用这一行**：状态改回 `enabled`、先把它名下仍为 `bound` 的旧令牌改成 `revoked`（否则手机一恢复，旧快捷指令跟着复活），再领一个新令牌。照常新建会撞唯一索引。要一条全新的记录就用删除（软删），唯一索引带 `deleted_at`，删掉之后同一个号码可以直接重绑。
...
**唯一索引下探到「账号 + 项目」这一层，同一个号可以被不同的人、不同的项目各绑一次。**

因为**手机往往不是本站用户自己的，而是他客户的**：甲、乙两家服务商可能服务同一位客户，
各自给那部手机装自己的快捷指令。号码若在租户内唯一，先绑的那家把号占死，后来的一家连
登记都做不到——而他们本来就该互不相干。
...
同一账号在同一项目下重复绑定同一个号码必须幂等：直接返回成功，不创建重复的有效记录——
靠的是上面那条唯一索引加 `ignoreInsert`，不是先查后插（理由同 §4.6）。
...
**同一个号码可以被不同的人、不同的项目各登记一次**，唯一索引是 `[owner_tid, owner_uid, integration_client_id, number, deleted_at]`。
...
| 手机号格式错误 | 手机号码格式不正确 |
| 手机已在**该接入方项目下**绑给本账号 | 幂等返回成功，不产生重复记录 |

手机已在同一项目下绑给当前账号时按幂等处理：返回成功，不产生重复记录。**别的账号绑过
同一个号不算冲突**，理由见 §4.3——两家服务商服务同一位客户是常事。
