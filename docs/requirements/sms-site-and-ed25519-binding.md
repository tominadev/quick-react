...
- **自检**：手机的主人手动运行快捷指令时没有短信触发，`content`、`sender`、`message_id` 三个都为空。这时回 200，一句话告诉他这份快捷指令绑的是哪个号码（完整号码加设备名称，不打码——持有令牌者本就能收到这部手机的全部短信），并刷新 `sms_phones.last_check_at`；不写 `sms_messages`、不刷新令牌的 `last_used_at`、不推送。回执不提推送地址与项目：读它的是手机的主人，那是平台用户的概念，放在后台「我的手机」里提示。
