...
**票据路径（§6.2）绑定时关联是强制的**：`integration_client_id` 从票据里的 `client_id` 来，由服务端验签后写入，不由调用方指定——自称可以伪造，而这一列决定了短信将来推给谁。会话路径（用户在控制台自己绑）才是可选的：他在下拉里挑一个自己的项目，或者不挑，那就是自己的手机。
...
**四种「找不到可用公钥」的成因共用一句话**（接入方不存在 / 已停用 / kid 不存在 / kid 已退役）：验签发生在查到公钥之后，此刻调用方还未经认证，分开回答等于给出一个枚举平台上有哪些接入方的探测器。但那句话要点明**该查哪两个字段**，否则接入方手里握着四个可能只能挨个试。

| 情况 | 提示 |
| --- | --- |
| `client_id` 或 `kid` 不存在、已禁用、已 `retired` | 签名接入方无效：client_id 与 kid 没有匹配到一把启用中的公钥 |
