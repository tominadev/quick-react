...
| `client_ref` | 接入方自己的引用串（票据外层字段，不进签名），默认空字符串，**不参与任何唯一性判断**。推送短信时原样带回给接入方（§4.9.2）——接入方不一定按手机号存自己的客户 |
...
**票据路径（§6.2）的 `key`、`client_ref` 都在票据外层，不由服务端验证是不是"真的"属于
哪个客户**——它们是接入方自己的标签，`client_ref` 原样存、原样推回去，不解析、不用于任何
权限判定；`key` 只用于去重与归属比对，同样不解析其内容。
...
`key`（接入方自己指定的行标识，驱动去重）、`client_ref`（接入方自己的引用串，随推送带回）、
`title`（给手机起的名字）都在票据**外层**、随 HTTP 请求体一起提交，不进上面这段签名
JSON——它们不是授权的一部分，只是随请求带来的元数据，和 `public_key`/`phone`/`nonce`
这些决定"谁、绑哪个号、能不能重放"的字段不是一类东西。`key` 详见 §4.3。
