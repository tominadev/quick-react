...
| 面 | 路径 | 页面 | 写入 |
| --- | --- | --- | --- |
| 管理面 | `/panel/admin/sms/` | 生成器机器、Shortcut 令牌、接入方 | 进审批队列 |
| 用户面 | `/panel/user/sms/` | 我的手机、我的短信 | 立即生效 |
...
- **接入方**能新增，但公钥不在这一页——轮换要新旧并存，拆在 `sms_integration_client_keys`
  （§4.2）。公钥登记随 `/api/client/` 那条链一起做：没有验签入口时先登记公钥没有用处。
