...
4. 页面通过统一 API 提交票据；后端解析票据，**按票据里的 `public_key` 反查出接入方与归属账号**，再校验签名、`aud`、时间窗、手机号格式。
...
| `public_key` | 对应 `sms_integration_client_keys.public_key`，**全库唯一**。票据里唯一的身份字段：验签公钥、接入方、归属账号全由它反查 |
| `phone` | 规范化后的 E.164 手机号，对应 `sms_phones.number` |
| `iat`、`exp` | Unix 秒；有效期不超过 5 分钟，允许的时钟偏差不超过 60 秒 |
| `nonce` | 接入方生成的高熵随机数，同一接入方不得重复 |
| `key` | 可选，接入方自己指定的行标识，驱动去重，详见 §4.3 |
| `client_ref` | 可选，接入方自己的引用串，随推送带回，不参与去重 |
| `title` | 可选，给手机起的名字 |

**请求体只有 `ticket` 一个字段，没有票据外层。** `key`/`client_ref`/`title` 都在上面这段
签名 JSON 里，不设"签名之外还能带元数据"这个口子——每加一个新的可选字段，不用判断它
该不该进签名，规则永远只有一条：**要发给服务端的，都在这段被签名、被验证的 JSON 里**。
这也堵住了一类问题：如果这些字段能在签名之外自由提交，中间的任何一环（网络、代理、接入方
自己的转发服务）理论上都能在不破坏签名的前提下篡改它们——全部纳入签名之后，篡改任何一个
字节都会让验签失败。
...
**票据里的身份字段只有 `public_key`**（同 GitHub 的 SSH 公钥）：服务端据它反查接入方与归属账号，调用方不填 `client_id`、`kid`、`base_user_id`。
