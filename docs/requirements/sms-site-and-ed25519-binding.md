...
| `secret_encrypted` | 签名密钥，**加密存储而非哈希** |
| `phone_id` | 限定只推送该手机的短信；为空表示该账号全部手机 |
| `status` | `enabled` / `disabled` |
| `last_success_at`、`last_error` | 最近一次成功时间与最近错误摘要 |

**密钥必须可解密，不能沿用 Access Key 的"只存哈希"做法。** Access Key 是服务端*校验*来访者，哈希足够；推送密钥是服务端*自己签名*，必须拿到明文。二者性质相反，照抄会导致签名根本无法实现。密钥用平台密钥加密后入库，轮换时生成新值并允许短期并存。
...
X-Sms-Signature: sha256=<hex>
```

签名为 `HMAC-SHA256(secret, timestamp + "." + 原始请求体字节)`，输出小写十六进制，与 §10 的编码规则一致。接收方应校验时间戳在合理窗口内（建议 5 分钟）以防重放，并按 `X-Sms-Delivery-Id` 去重。
...
| 推送签名密钥 | SMS 与用户服务端共享 | 为推送请求签名 | **加密存储，可解密** |

最后一行是唯一的例外，也是最容易写错的一处：前三种服务端只做*校验*，哈希足够；推送密钥服务端要*自己签名*，必须能取回明文。照抄"只存哈希"会导致签名无法实现，见 §4.9.1。

五者不能互相替代。平台预配凭证不属于任何账号，不下发给用户，不能调用短信接收或账号绑定接口。Ed25519 只证明"登记的服务端持有私钥并签署了该票据"，不替代用户登录会话，也不替代手机令牌认证。
