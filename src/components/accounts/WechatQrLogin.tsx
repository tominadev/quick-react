...
export default function WechatQrLogin({ commonApi }: { commonApi: CommonApi }) {
	...
	const [bindUrl, setBindUrl] = useState(''), [fallbackUrl, setFallbackUrl] = useState(''), [email, setEmail] = useState(''), [code, setCode] = useState(''), [submitting, setSubmitting] = useState(false);
	...
	const load = (refresh = false) => {
		...
		requestQr(commonApi, refresh).then((data) => {
			setFallbackUrl(typeof data.fallbackUrl === 'string' ? data.fallbackUrl : '');
	...
	return <main style={{ maxWidth: 520, margin: '80px auto', padding: 24, textAlign: 'center' }}><h2>微信扫码登录</h2><div ref={qrRef} style={{ display: state === 'ready' ? 'inline-block' : 'none', margin: 24 }} />{state === 'email' ? <><Alert type="info" showIcon message="微信身份已确认，请输入用于 Accounts 的邮箱。验证码只会发送到你填写的地址。" style={{ marginTop: 20, marginBottom: 16, textAlign: 'left' }} /><Space.Compact style={{ maxWidth: 420, width: '100%' }}><Input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="请输入邮箱" disabled={submitting} /><Button type="primary" loading={submitting} onClick={() => submit('email')}>发送验证码</Button></Space.Compact></> : null}{state === 'code' ? <><Alert type="info" showIcon message={message} style={{ marginTop: 20, marginBottom: 16, textAlign: 'left' }} /><Space.Compact style={{ maxWidth: 420, width: '100%' }}><Input value={code} onChange={(event) => setCode(event.target.value)} placeholder="请输入 6 位验证码" maxLength={6} disabled={submitting} /><Button type="primary" loading={submitting} onClick={() => submit('verify')}>验证邮箱</Button></Space.Compact><Button type="link" disabled={submitting} onClick={() => { setState('email'); setCode(''); setMessage('请重新输入接收验证码的邮箱'); }}>更换邮箱</Button></> : null}{state !== 'email' && state !== 'code' ? <Alert type={state === 'error' || state === 'expired' ? 'error' : 'info'} showIcon message={message} style={{ marginTop: 20 }} /> : null}<Space style={{ marginTop: 16 }}><>{state === 'expired' || state === 'error' ? <Button onClick={() => load(true)}>刷新二维码</Button> : null}</><>{fallbackUrl ? <Button onClick={() => window.location.assign(fallbackUrl)}>更换邮箱或使用 Google 登录</Button> : null}</></Space></main>;
}
