...
import { allSql, firstSql, sql, type SqlSortOption } from '@server/database/sql.mjs';
...
const oidcClientColumns = { id: 'client_id', title: 'title', secret_hash: 'secret_hash', redirect_uris: 'redirect_uris', backchannel_logout_uri: 'backchannel_logout_uri', allowed_scopes: 'allowed_scopes', require_pkce: 'require_pkce', strict_redirect_uri: 'strict_redirect_uri', password_sync: 'password_sync', status: 'status', created_at: 'created_at', updated_at: 'updated_at' } as const;
export const oidcClients = (database: DatabaseAdapter, sort?: SqlSortOption) => allSql<OidcClientRecord>(database, sql({ database }).select({ table: 'passport_oidc_clients', columns: oidcClientColumns, sort, orderBy: [{ column: 'created_at', direction: 'DESC' }] }));
...
export const authorizationCode = (database: DatabaseAdapter, hash: string) => firstSql<AuthorizationCodeRecord>(database, sql({ database }).select({ table: 'passport_oidc_authorization_codes', columns: { client_id: 'client_id', user_key: { column: 'user_key', cast: 'text' }, redirect_uri: 'redirect_uri', scope: 'scope', nonce: 'nonce', code_challenge: 'code_challenge', code_challenge_method: 'code_challenge_method', expires_at: 'expires_at', consumed_at: 'consumed_at', session_id: 'session_id' }, where: [{ column: 'code_hash', value: hash }] }));
...
export const accountUser = async (database: DatabaseAdapter, userId: string) => {
	...
	const row = await firstSql<{ sub: string; user_name: string; profile_nickname: string | null; status: string }>(database, sql({ database }).select({
		...
		columns: { sub: { column: 'u.key', cast: 'text' }, user_name: 'u.name', profile_nickname: 'p.nickname', status: 'u.status' },
		joins: [{ type: 'LEFT', table: 'passport_user_profiles', alias: 'p', left: 'p.user_key', right: 'u.key' }],
		where: [{ column: 'u.key', value: userId }],
	}));
	if (!row) return null;
...
export const accountCredentialClaim = async (database: DatabaseAdapter, userId: string) => {
	const row = await firstSql<{ password: unknown }>(database, sql({ database }).select({
		...
		where: [{ column: 'user_key', value: userId }], limit: 1,
	}));
	return row?.password;
};

export const accessTokenUser = async (database: DatabaseAdapter, tokenHash: string, now: number) => {
	const token = await firstSql<{ user_key: string }>(database, sql({ database }).select({ table: 'passport_oidc_access_tokens', alias: 't', columns: { user_key: { column: 't.user_key', cast: 'text' } }, joins: [{ table: 'passport_users', alias: 'u', left: 'u.key', right: 't.user_key' }], where: [{ column: 't.token_hash', value: tokenHash }, { column: 't.expires_at', operator: '>', value: now }, { column: 't.revoked_at', operator: 'IS NULL' }, { column: 'u.status', value: 'enabled' }] }));
	return token ? accountUser(database, token.user_key) : null;
};

export const passportSessionUser = async (database: DatabaseAdapter, sessionId: string) => firstSql<{ user_key: string }>(database, sql({ database }).select({ table: 'passport_sessions', columns: { user_key: { column: 'user_key', cast: 'text' } }, where: [{ column: 'token_hash', value: await sha256(sessionId) }] }));
