...
export const loadPassportSession = async (database: DatabaseAdapter, request: Request) => {
	...
	const user = await firstSql<{ user_key: string; user_name: string; profile_nickname: string | null; device_id?: string | null }>(database, sql({ database }).select({ table: 'passport_sessions', alias: 's', columns: { user_key: { column: 'u.key', cast: 'text' }, user_name: 'u.name', profile_nickname: 'p.nickname', device_id: { column: 's.device_id', cast: 'text' } }, joins: [{ table: 'passport_users', alias: 'u', left: 'u.key', right: 's.user_key' }, { type: 'LEFT' as const, table: 'passport_user_profiles', alias: 'p', left: 'p.user_key', right: 's.user_key' }], where: [{ column: 's.token_hash', value: sessionHash }, { column: 's.expires_at', operator: '>', value: Date.now() }, { column: 'u.status', value: 'enabled' }] }));
	...
	try {
		if (await validatePassportDevice(database, user.user_key, user.device_id, request)) // 没设过资料就回落到用户名。
			return { id: user.user_key, user_name: user.user_name, profile_nickname: passportProfileNicknameOf(user.user_name, user.profile_nickname), roles: [] };
	} catch {
		// 指纹格式错误同样使当前会话失效。
	}
	await runSql(database, sql({ database }).delete('passport_sessions', { token_hash: sessionHash }));
...
export const loadPassportDeviceUserId = async (database: DatabaseAdapter, request: Request): Promise<string | number | bigint | null> => {
	...
	const sessionHash = await sha256(sessionId);
	const session = await firstSql<{ user_key: string; device_id: string | null }>(database, sql({ database }).select({
		table: 'passport_sessions',
		columns: { user_key: { column: 'user_key', cast: 'text' }, device_id: { column: 'device_id', cast: 'text' } },
	...
	const binding = await firstSql<{ id: string }>(database, sql({ database }).select({
		...
		where: [{ column: 'device_id', value: session.device_id }, { column: 'user_key', value: session.user_key }, { column: 'status', value: 'active' }],
		limit: 1,
	}));
