...
const pendingOtp = (database: DatabaseAdapter, identity: TelegramIdentity) => firstSql<{ email: string }>(database, sql({ database }).select({ table: 'passport_telegram_email_otps', columns: { email: 'email' }, where: [{ column: 'bot_id', value: identity.botId }, { column: 'telegram_user_id', value: identity.telegramUserId }, { column: 'status', value: 'pending' }], orderBy: [{ column: 'created_at', direction: 'DESC' }, { column: 'id', direction: 'DESC' }], limit: 1 }));
const latestOtp = (database: DatabaseAdapter, identity: TelegramIdentity) => firstSql<{ email: string }>(database, sql({ database }).select({ table: 'passport_telegram_email_otps', columns: { email: 'email' }, where: [{ column: 'bot_id', value: identity.botId }, { column: 'telegram_user_id', value: identity.telegramUserId }], orderBy: [{ column: 'created_at', direction: 'DESC' }, { column: 'id', direction: 'DESC' }], limit: 1 }));
