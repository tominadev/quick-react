...
export const createDatabaseConfigStore = (database: DatabaseAdapter): ConfigStore => ({
	get: async (key) => {
		const row = await firstSql<{ value: string }>(database, sql({ database }).select({ table: 'base_configs', columns: { value: 'value' }, where: [{ column: 'key', value: key }] }));
	...
	put: async (key, value) => {
		await runSql(database, sql({ database }).upsert('base_configs', ['key'], { key, value: JSON.stringify(value) }, ['value', 'updated_at']));
	},
});
