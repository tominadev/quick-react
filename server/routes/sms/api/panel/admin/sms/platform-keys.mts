...
const handler: ApiHandler = async (c, next, params) => {
	...
	if (c.req.method === 'GET' && !params.id) {
		...
		return apiResponse(c, 200, { table: {
			option: { rowKey: 'id', actions: {
				...
				toolbar: [{ key: 'generate', label: '生成新密钥', confirm: '生成一把新密钥并立即用于签名，当前这把转为「退役中」（仍然公布给接收方）。确认吗？' }],
				row: [{ key: 'retire', label: '退役', confirm: '退役之后这把公钥不再公布，用它签过、还在重试的请求会验不过。确认吗？', visibleWhen: { field: 'status', values: ['retiring'] } }],
			} },
	 ...
	 * 生成一把新密钥并切换。
	 *
	 * **先插新的、再把旧的转 retiring**，两条语句一次提交（同一个 operation_id，批准时
	 * 一起生效）。反过来的话中间会有一瞬间**一把 active 都没有**，那时候来的短信签不了名、
	 * 投递直接失败；而这个顺序的中间态是两把 active，签名取最新那把，不影响任何一次投递。
	 */
	if (!params.id && c.req.method === 'POST' && c.req.query('action') === 'generate') {
		...
		await runOperation(c, database, [
			sql({ database }).insert('sms_platform_keys', { kid: generated.kid, public_key: generated.publicKey, private_key: generated.privateKey, status: 'active' }),
			...active.map((row) => sql({ database }).update('sms_platform_keys', { status: 'retiring' }, [{ column: 'id', value: row.id }, { column: 'status', value: 'active' }])),
		]);
		return apiMessage(c, 201, `新密钥已生成（${generated.kid}）。旧的那把转为「退役中」仍会公布，等过一个完整的投递重试窗口再退役它。`);
	}
