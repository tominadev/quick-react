...
const transferGroups = async (database: DatabaseAdapter, siteKey: string) => {
	...
	for (let depth = 0; current && current !== 'base' && depth < 8; depth += 1) {
		...
		const parent: { base_site_key: string | null } | null = await firstSql<{ base_site_key: string | null }>(database, sql({ database }).select({ table: 'global_sites', columns: { base_site_key: 'base_site_key' }, where: [{ column: 'key', value: current }] }));
...
const validateParent = async (database: DatabaseAdapter, siteKey: string, parentSiteKey: string) => {
	...
	for (let depth = 0; depth < 8 && current !== 'base'; depth += 1) {
		...
		visited.add(current);
		const parent = await firstSql<{ base_site_key: string | null; is_system: number }>(database, sql({ database }).select({ table: 'global_sites', columns: { base_site_key: 'base_site_key', is_system: 'is_system' }, where: [{ column: 'key', value: current }] }));
...
const handler: ApiHandler = async (c, next, params) => {
	...
	if (!params.id && c.req.method === 'DELETE') {
		...
		for (const siteKey of siteKeys) {
			...
			await runSql(database, sql({ database }).delete('global_sites', { key: siteKey }));
		}
		await c.get('siteRouter').refresh();
		return apiMessage(c, 200, '删除成功');
	}
	if (params.id && c.req.method === 'GET') {
		const row = await firstSql<Record<string, unknown>>(database, sql({ database }).select({ table: 'global_sites', columns: siteColumns, where: [{ column: 'key', value: params.id }] }));
		return row ? apiResponse(c, 200, publicSite(row)) : apiMessage(c, 404, '站点不存在');
	}
	if (params.id && c.req.method === 'POST') {
		const action = c.req.query('action')?.trim() || 'migrate';
		const site = await firstSql<{ site_key: string; dsn: string; database_binding: string; is_system: number }>(database, sql({ database }).select({ table: 'global_sites', columns: { site_key: 'key', dsn: 'dsn', database_binding: 'database_binding', is_system: 'is_system' }, where: [{ column: 'key', value: params.id }] }));
	...
	if (params.id && c.req.method === 'PUT') {
		const current = await firstSql<{ site_key: string; is_system: number; dsn: string; database_binding: string; base_site_key: string | null; migration_status: string }>(database, sql({ database }).select({ table: 'global_sites', columns: { site_key: 'key', is_system: 'is_system', dsn: 'dsn', database_binding: 'database_binding', base_site_key: 'base_site_key', migration_status: 'migration_status' }, where: [{ column: 'key', value: params.id }] }));
	...
	if (params.id && c.req.method === 'DELETE') {
		...
		await runSql(database, sql({ database }).delete('global_sites', { key: params.id }));
		await c.get('siteRouter').refresh();
		return apiMessage(c, 200, '删除成功');
	}
	return next();
