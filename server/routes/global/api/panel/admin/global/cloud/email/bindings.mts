...
const listOptions = async (database: DatabaseAdapter) => {
	const [sites, channels, templates] = await Promise.all([
		...
		allSql<{ id: number; account_name: string; region: string; credential_title: string; provider: string }>(database, sql({ database }).select({ table: 'global_cloud_email_channels', alias: 'ch', columns: { id: 'ch.id', account_name: 'ch.account_name', region: 'ch.region', credential_title: 'c.title', provider: 'c.provider' }, joins: [{ table: 'global_cloud_credentials', alias: 'c', left: 'c.id', right: 'ch.cloud_credential_id' }], where: [{ column: 'ch.status', value: 'enabled' }, { column: 'c.status', value: 'enabled' }], orderBy: [{ column: 'c.title' }, { column: 'ch.account_name' }] })),
		allSql<{ id: number; template_key: string; template_type: string; name: string }>(database, sql({ database }).select({ table: 'global_cloud_email_templates', columns: { id: 'id', template_key: 'key', template_type: 'type', title: 'title' }, where: [{ column: 'status', value: 'enabled' }], orderBy: [{ column: 'type' }, { column: 'key' }] })),
	]);
	return {
		sites: sites.map((item) => ({ value: item.site_key, text: `${item.title} (${item.site_key})` })),
		channels: channels.map((item) => ({ value: String(item.id), text: `${item.credential_title} / ${getCloudEmailProduct(item.provider)} / ${item.account_name} / ${getCloudEmailRegionLabel(item.provider, item.region)}（${item.region}）` })),
...
const validateTarget = async (database: DatabaseAdapter, siteKey: string, channelId: number, templateId: number, enabled: boolean): Promise<TargetValidation> => {
	...
	const [site, channel, template] = await Promise.all([
		...
		firstSql<{ id: number; template_type: string }>(database, sql({ database }).select({ table: 'global_cloud_email_templates', columns: { id: 'id', template_type: 'type' }, where: [{ column: 'id', value: templateId }, { column: 'status', value: 'enabled' }] })),
	]);
...
const clearOtherDefaults = (c: Context<AppEnv>, database: DatabaseAdapter, id: number, siteKey: string, purpose: string) => runOperationSql(c, database, sql({ database }).update('global_cloud_email_bindings', { is_default: false }, [{ column: 'site_key', value: siteKey }, { column: 'purpose', value: purpose }, { column: 'id', operator: '!=', value: id }, { column: 'is_default', value: true }]));
...
const handler: ApiHandler = async (c, next, params) => {
	...
	if (!params.id && c.req.method === 'GET') {
		const [rows, options] = await Promise.all([
			allSql<Record<string, unknown>>(database, sql({ database }).select({ table: 'global_cloud_email_bindings', alias: 'b', columns: { id: 'b.id', site_key: 'b.site_key', site_title: 's.title', channel_id: 'b.channel_id', account_name: 'ch.account_name', region: 'ch.region', credential_title: 'c.title', provider: 'c.provider', template_id: 'b.template_id', template_key: 't.key', template_title: 't.title', purpose: 'b.purpose', is_default: 'b.is_default', status: 'b.status', created_at: 'b.created_at', updated_at: 'b.updated_at' }, joins: [{ table: 'global_sites', alias: 's', left: 's.key', right: 'b.site_key' }, { table: 'global_cloud_email_channels', alias: 'ch', left: 'ch.id', right: 'b.channel_id' }, { table: 'global_cloud_credentials', alias: 'c', left: 'c.id', right: 'ch.cloud_credential_id' }, { table: 'global_cloud_email_templates', alias: 't', left: 't.id', right: 'b.template_id' }], sort: tableSort(c), orderBy: [{ column: 'b.id', direction: 'DESC' }] })),
	...
	if (!params.id && c.req.method === 'POST') {
		...
		try {
			...
			if (isDefault && database.batch) await database.batch([
				insert,
				builder.update('global_cloud_email_bindings', { is_default: false }, { site_key: siteKey, purpose, is_default: true }),
				builder.update('global_cloud_email_bindings', { is_default: true }, { site_key: siteKey, channel_id: channelId, template_id: templateId, purpose }),
			]);
	...
	if (params.id && c.req.method === 'PUT') {
		...
		try {
			...
			if (isDefault && database.batch) await database.batch([
				update,
				builder.update('global_cloud_email_bindings', { is_default: false }, [{ column: 'site_key', value: siteKey }, { column: 'purpose', value: purpose }, { column: 'id', operator: '!=', value: Number(params.id) }, { column: 'is_default', value: true }]),
				builder.update('global_cloud_email_bindings', { is_default: true }, { id: Number(params.id) }),
			]);
			else {
