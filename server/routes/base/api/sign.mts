...
const localSign: ApiHandler = async (c, next) => {
	...
	if (c.req.method === 'PUT') {
		...
		// 影响 0 行说明已被并发请求抢先。
		//
		// 引导流程不留痕：那时还没有会话，操作者与作用账号都是空的，而 base_bootstraps.value
		// 是脱敏列（§5），记下来只会是一条「value：已变更」——既说不出谁，也说不出改了什么。
		// 「初始管理员是什么时候建的」由 base_users.created_at 回答，不需要再抄一遍。
		const claimed = await runSql(systemDatabase, sql({ database: systemDatabase }).update('base_bootstraps', { value: 'claimed' }, [{ column: 'name', value: 'initial_admin' }, { column: 'value', value: 'open' }, ...(tenantId === null ? [] : [{ column: 'owner_tid', value: tenantId }])]));
		...
		try {
			...
			// 回滚本租户的认领，让下一次注册还能重试。
			await runSql(systemDatabase, sql({ database: systemDatabase }).update('base_bootstraps', { value: 'open' }, [{ column: 'name', value: 'initial_admin' }, { column: 'value', value: 'claimed' }, ...(tenantId === null ? [] : [{ column: 'owner_tid', value: tenantId }])]));
			throw error;
		}
