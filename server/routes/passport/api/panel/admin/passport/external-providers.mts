...
type ProviderRow = { id: number; provider: ProviderId; title: string; client_id: string; client_secret: string; wechat_mode: WechatMode; wechat_redirect_domain: string; status: string; created_at: number; updated_at: number };
...
const columns = [
	...
	{ dataIndex: 'title', title: '显示名称', component: 'textbox', rules: [{ required: true, message: '请输入显示名称' }] },
...
const handler: ApiHandler = async (c, next, params) => {
	...
	if (!params.id && c.req.method === 'POST') {
		...
		const id = providerId(body.provider), displayName = requiredText(body.title), clientId = requiredText(body.client_id), clientSecret = requiredText(body.client_secret), wechatMode: WechatMode = body.wechat_mode === 'official_account' ? 'official_account' : 'open_platform', redirectDomain = requiredText(body.wechat_redirect_domain).replace(/^https?:\/\//, '').replace(/\/.*$/, '');
	...
	if (c.req.method === 'PUT') {
		...
		const displayName = requiredText(body.title), clientId = requiredText(body.client_id), clientSecret = requiredText(body.client_secret), wechatMode: WechatMode = body.wechat_mode === 'official_account' ? 'official_account' : 'open_platform', redirectDomain = requiredText(body.wechat_redirect_domain).replace(/^https?:\/\//, '').replace(/\/.*$/, '');
		if (!displayName || !clientId) return apiMessage(c, 400, '显示名称和客户端 ID 不能为空');
		if (id === 'wechat' && redirectDomain && !/^(?=.{1,253}$)([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}$/i.test(redirectDomain)) return apiMessage(c, 400, '微信授权回调域名格式不正确，请只填写域名');
		const updated = await runOperationSql(c, database, sql({ database }).update('passport_external_providers', { title: displayName, client_id: clientId, client_secret: clientSecret || undefined, wechat_mode: wechatMode, wechat_redirect_domain: id === 'wechat' ? redirectDomain : '', status: body.status === 'disabled' ? 'disabled' : 'enabled' }, { provider: id }));
		return Number(updated.meta?.changes ?? 0) ? apiMessage(c, 200, '外部身份源已保存') : apiMessage(c, 404, '外部身份源不存在');
	}
	if (c.req.method === 'DELETE') {
		const current = await firstSql<{ status: string }>(database, sql({ database }).select({ table: 'passport_external_providers', columns: { status: 'status' }, where: [{ column: 'provider', value: id }] }));
		...
		await runOperationSql(c, database, sql({ database }).softDelete('passport_external_providers', { provider: id }));
