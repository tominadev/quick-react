...
export default pveCrud({ table: 'pve_nodes', key: 'id', columns, writable: ['region_id', 'name', 'host', 'port', 'cluster_name', 'api_user', 'api_token_id', 'api_token_secret', 'status'], prepareColumns: async (database) => {
	const regions = (await allSql<{ id: number; key: string; title: string; status: unknown }>(database, sql({ database }).select({ table: 'pve_regions', columns: { id: 'id', key: 'key', title: 'title', status: 'status' }, orderBy: [{ column: 'sort_order' }, { column: 'id' }] })))
	...
	return columns.map((column) => column.dataIndex === 'region_id' ? { ...column, options: regions.map((region) => ({ value: String(region.id), text: `${region.title} (id:${region.id}, key:${region.key})` })) } : column);
} });
